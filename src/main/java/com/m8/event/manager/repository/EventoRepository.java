...
//     List<EventoRepository> findByFecha (Date fecha); 
    ...
    @Query("SELECT e FROM Evento e WHERE e.fechaInicio >= CURRENT_DATE()")
    public List<Evento> buscarProximosEventos ();
    
    @Query("SELECT e FROM Evento e WHERE e.dias = :dia")
    public List<Evento> buscarPorDiaSemana (@Param("dia") Enum Dia);
